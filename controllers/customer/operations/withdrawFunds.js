...
const withdrawFunds = async (req, res) => {
  ...
  try {
    ...
    if (amount <= verifyAccount.totalBalance) {
      await Account.update({ totalBalance: verifyAccount.totalBalance - amount },
        { where: { accountNo: verifyAccount.accountNo } });
    } else {
      return res.status(400).json({ msg: "Insufficient balance in the given bank account. Please try again." })
    }

    const currentBalance = await Account.findOne({ attributes: ['totalBalance'], where: { accountNo: verifyAccount.accountNo } });

    const updatedAccountInfo = {
      withdrawl: verifyAccount.email,
